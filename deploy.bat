...
start /b "" cmd /c "echo 🌐 [Web] Compiling... && flutter build web --release >nul 2>&1 && echo ☁️ [Web] Deploying to Vercel... && pushd build\web && vercel link --yes >nul 2>&1 && vercel --prod --yes --archive=tgz >nul 2>&1 && popd && echo ✅ [Web] Web Deployment Complete! && echo done > "%~dp0web_done.tmp"" || (echo done > "%~dp0web_failed.tmp" && exit /b 1)"
