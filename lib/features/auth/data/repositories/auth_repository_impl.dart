...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<void> sendOtp({required String identifier, bool requireApprovedSignup = true}) async {
    ...
    try {
      ...
      if (e.message.contains('User not found')) throw Exception('No account found for this phone number.');
      rethrow;
    }
  }
  ...
  Future<void> verifyOtp({required String identifier, required String otp}) async {
    try {
      ...
      try {
        ...
        if (kDebugMode) print("phoneChange verification failed, attempting standard sms fallback: ${e.message}");
      ...
      if (response.user != null) {
        ...
        if (!profileExists && kDebugMode) print("Warning: Profile record still not found after retries.");
      }
    } on AuthException catch (e) {
      throw e.message;
    } catch (e) {
  ...
  Future<void> updateProfile({
    ...
    if (!isAdmin && qualification != null) updateData['qualification'] = qualification;
