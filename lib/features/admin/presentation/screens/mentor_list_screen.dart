...
class _MentorListScreenState extends ConsumerState<MentorListScreen> {
  ...
  Widget _buildMentorCard(Map<String, dynamic> mentor) {
    final isSenior =
        mentor['role']?.toString().toLowerCase().contains('senior') ?? false;
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text(mentor['role']?.toString().toUpperCase() ?? "MENTOR"),
        children: [
          _infoTile(Icons.phone, "Phone", mentor['phone']),
          _infoTile(Icons.location_on, "Village", mentor['village']),
          _infoTile(Icons.hub, "Cluster", mentor['cluster']),
          _infoTile(Icons.school, "School", mentor['school']),
          _infoTile(
            Icons.history_edu,
            "Qualification",
